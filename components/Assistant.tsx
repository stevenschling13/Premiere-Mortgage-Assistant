...
import type { VariableSizeList as VariableSizeListType, ListChildComponentProps } from 'react-window';
...
export const Assistant: React.FC = () => {
  ...
  const messagesToRender = isSimulationMode ? simulationMessages : messages;
  ...
  useEffect(() => {
    const computeHeight = () => {
        ...
        const reserve = isLiveMode ? 360 : 260;
        setListHeight(Math.max(320, window.innerHeight - reserve));
    };
  ...
  useEffect(() => {
    if (!listRef.current || messagesToRender.length === 0) return;
    listRef.current.scrollToItem(Math.max(0, messagesToRender.length - 1), 'end');
  }, [messagesToRender.length, isSimulationMode]);
  ...
  const getMessageSize = useCallback((index: number) => {
    ...
    let base = msg.text.length > 400 ? 220 : msg.text.length > 200 ? 160 : 110;

    if (msg.verificationResult) base += 140;
    if ((msg.groundingLinks?.length || 0) > 0) base += 80;
    if (msg.searchEntryPoint) base += 40;
    if (msg.role !== 'user') base += 20;
  ...
  const renderMessageRow = useCallback(
    ({ index, style }: ListChildComponentProps) => {
      ...
      return (
        <div style={style} className="px-1 md:px-2 py-1">
          <div
    ...
    [messagesToRender, isSimulationMode, verifyingMsgIds, handleVerifyMessage]
  );
  ...
  return (
    <div className="h-[calc(100dvh-64px)] md:h-[calc(100dvh-32px)] flex flex-col bg-white md:rounded-xl shadow-sm border-t md:border border-gray-200 overflow-hidden mx-auto max-w-5xl md:my-4 relative w-full animate-fade-in">
      ...
      {/* Messages Area */}
      <div className={`flex-1 overflow-hidden p-4 md:p-6 ${isSimulationMode ? 'bg-indigo-50/30' : 'bg-gray-50/50'}`}>
        <VirtualizedList
          ref={listRef}
          height={listHeight}
          width={'100%'}
          ...
          itemKey={(index) => messagesToRender[index]?.id ?? `message-${index}`}
          overscanCount={4}
