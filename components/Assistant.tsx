...
const VirtualList = (ReactWindow as any).List as React.ComponentType<any>;
...
export const Assistant: React.FC = () => {
  ...
  useEffect(() => {
    ...
    listRef.current.scrollToRow({ index: Math.max(0, messagesToRender.length - 1), align: 'end' });
  ...
  useEffect(() => {
    const computeHeight = () => {
        ...
        const reserve = isLiveMode ? 360 : 260; // extra space for voice UI/input
  ...
  const renderMessageRow = useCallback(
    ...
    [messagesToRender, isSimulationMode, verifyingMsgIds]
  );
    ...
    <div className="h-[calc(100dvh-64px)] md:h-[calc(100dvh-32px)] flex flex-col bg-white md:rounded-xl shadow-sm border-t md:border border-gray-200 overflow-hidden mx-auto max-w-5xl md:my-4 relative w-full animate-fade-in">
      ...
      <div className={`flex-1 overflow-hidden p-4 md:p-6 ${isSimulationMode ? 'bg-indigo-50/30' : 'bg-gray-50/50'}`}>
        <VirtualList
          ...
          rowHeight={getMessageSize}
          rowComponent={({ index, style }: { index: number; style: React.CSSProperties }) => renderMessageRow({ index, style })}
          listRef={listRef}
          overscanCount={4}
        />
        {isLoading && (
