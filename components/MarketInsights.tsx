...
import { 
    ...
    Linkedin, Mail, MessageSquare, Image as ImageIcon, 
    Layers, Loader2, Sparkles, Copy, ExternalLink, Download, 
    Palette, Trash2, Save, ShieldCheck, AlertTriangle, Link as LinkIcon, CheckCircle2, Home, Wallet
} from 'lucide-react';
import { 
...
import { MarketIndex, NewsItem, MarketingCampaign, GeneratedImage, VerificationResult } from '../types';
import { useToast } from './Toast';
import { MarkdownRenderer } from './MarkdownRenderer';

type Tab = 'PULSE' | 'CAMPAIGN' | 'VISUAL';
...
export const MarketingStudio: React.FC = () => {
  ...
  const updateMarketingState = (updates: Partial<MarketingState>) => {
      setMarketingState(prev => {
          ...
          // Determine if we should save images to storage (limit to last 2 to save space)
          const stateToSave = { 
              ...newState, 
              generatedImages: newState.generatedImages.slice(0, 2) 
          };
          saveToStorage(StorageKeys.MARKETING_DATA, stateToSave);
          return newState;
      });
  };

  const handleClearWorkspace = () => {
      if(confirm('Clear all campaign drafts and generated images?')) {
  ...
  const handleGenerateImage = async () => {
      if (!marketingState.imagePrompt) return;
      setIsGeneratingImage(true);
      try {
          const base64Url = await generateSocialImage(marketingState.imagePrompt);
          updateMarketingState({
              generatedImages: [{ url: base64Url, prompt: marketingState.imagePrompt }, ...marketingState.generatedImages]
          });
      } catch (error) {
  ...
  const handleDownloadImage = (url: string, index: number) => {
      ...
      link.href = url;
      link.download = `social-image-${index}.png`;
  ...
  const renderVisualStudio = () => (
      <div className="animate-fade-in space-y-6">
          <div className="bg-gradient-to-r from-purple-900 to-indigo-900 rounded-xl shadow-lg p-6 text-white relative overflow-hidden">
              <div className="absolute top-0 right-0 p-10 opacity-10">
                  <Palette size={120} />
              </div>
              <div className="relative z-10">
                  <h3 className="font-bold text-xl mb-2 flex items-center">
                      <ImageIcon className="mr-2 text-purple-300" size={24}/>
                      AI Visual Studio
                  </h3>
                  <p className="text-purple-200 text-sm mb-6 max-w-lg">
                      Generate photorealistic, luxury-aesthetic images for your social media backgrounds. 
                      Powered by Gemini 3 Pro Image (Preview).
                  </p>
                  
                  <div className="flex gap-2">
                      <input 
                          value={marketingState.imagePrompt}
                          onChange={(e) => updateMarketingState({ imagePrompt: e.target.value })}
                          placeholder="Describe the image (e.g., Modern glass home on a cliffside at sunset)"
                          className="flex-1 p-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-purple-300 focus:ring-2 focus:ring-purple-400 outline-none backdrop-blur-sm"
                          onKeyDown={(e) => e.key === 'Enter' && handleGenerateImage()}
                      />
                      <button 
                          onClick={handleGenerateImage}
                          disabled={isGeneratingImage || !marketingState.imagePrompt}
                          className="px-6 py-3 bg-white text-purple-900 font-bold rounded-lg hover:bg-purple-50 transition-colors shadow-lg active:scale-95 disabled:opacity-50 flex items-center"
                      >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {marketingState.generatedImages.map((img, idx) => (
                  <div key={idx} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden group">
                      <div className="aspect-square relative bg-gray-100">
                          ...
                          <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-4">
                              ...
                              <button 
                                onClick={() => {
                                    updateMarketingState({ imagePrompt: img.prompt });
                                    window.scrollTo({top:0, behavior:'smooth'});
                                }}
                                className="p-2 bg-white rounded-full hover:bg-gray-100 text-gray-800" 
                                title="Use as prompt"
              ...
              {marketingState.generatedImages.length === 0 && !isGeneratingImage && (
                  <div className="col-span-3 py-12 text-center text-gray-400 border-2 border-dashed border-gray-200 rounded-xl">
                      ...
                      <p>No images generated yet. Try a prompt above.</p>
                  </div>
              )}
          </div>
      </div>
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Multi-channel campaign orchestration & asset creation.</p>
        </div>
        <div className="flex items-center space-x-2 bg-white p-1 rounded-xl border border-gray-200 shadow-sm">
            ...
            <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${activeTab === 'VISUAL' ? 'bg-brand-dark text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}
            >
                Visual Studio
            </button>
        </div>
      ...
      <div className="min-h-[500px]">
          ...
          {activeTab === 'VISUAL' && renderVisualStudio()}
      </div>
